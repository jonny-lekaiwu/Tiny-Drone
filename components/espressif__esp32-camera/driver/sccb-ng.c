...
/* 1: GPIO software SCCB through i2cdev; 0: original hardware SCCB below. */
#ifndef SCCB_USE_SOFTWARE_I2C
#define SCCB_USE_SOFTWARE_I2C 1
#endif

#if SCCB_USE_SOFTWARE_I2C

#include "i2cdev.h"

#define SCCB_FREQ CONFIG_SCCB_CLK_FREQ

static I2cDef sccb_sw_bus_def;
static I2C_Dev sccb_sw_bus = {
    .cfg = &sccb_sw_bus_def,
    .is_sw_i2c = true,
};
static bool sccb_sw_initialized;

static int sccb_sw_result(bool result)
{
    return result ? 0 : -1;
}
...
{
    ESP_LOGI(TAG, "software SCCB: SDA=%d SCL=%d", pin_sda, pin_scl);

    sccb_sw_bus_def.i2cPort = I2C_SW_CAMERA_PORT;
    sccb_sw_bus_def.i2cClockSpeed = SCCB_FREQ;
    sccb_sw_bus_def.scl_pin = pin_scl;
    sccb_sw_bus_def.sda_pin = pin_sda;
    sccb_sw_bus_def.gpioPullup = GPIO_PULLUP_ENABLE;

    i2cdevInit(&sccb_sw_bus);
    sccb_sw_initialized = sccb_sw_bus.init_ok;
    return sccb_sw_initialized ? ESP_OK : ESP_FAIL;
}

int SCCB_Use_Port(int i2c_num)
{
    (void)i2c_num;
    ESP_LOGE(TAG, "SCCB_Use_Port is unavailable with software SCCB");
    return ESP_ERR_NOT_SUPPORTED;
}

int SCCB_Deinit(void)
{
    /* i2cdev currently has no deinit API; leave the GPIO bus configured. */
    sccb_sw_initialized = false;
    return ESP_OK;
}

int SCCB_Probe(uint8_t slv_addr)
{
    return (sccb_sw_initialized &&
            i2cdevWrite(&sccb_sw_bus, slv_addr, 0, NULL)) ? ESP_OK : ESP_FAIL;
}

uint8_t SCCB_Read(uint8_t slv_addr, uint8_t reg)
{
    uint8_t data = 0xFF;
    if (!sccb_sw_initialized ||
        !i2cdevWrite(&sccb_sw_bus, slv_addr, 1, &reg) ||
        !i2cdevRead(&sccb_sw_bus, slv_addr, 1, &data)) {
        ESP_LOGE(TAG, "SCCB_Read failed addr=0x%02x reg=0x%02x", slv_addr, reg);
        return 0xFF;
    }
    return data;
}
...
{
    uint8_t tx[] = {reg, data};
    return sccb_sw_result(sccb_sw_initialized &&
                          i2cdevWrite(&sccb_sw_bus, slv_addr, sizeof(tx), tx));
}

uint8_t SCCB_Read16(uint8_t slv_addr, uint16_t reg)
{
    uint8_t addr[] = {(uint8_t)(reg >> 8), (uint8_t)reg};
    uint8_t data = 0xFF;
    if (!sccb_sw_initialized ||
        !i2cdevWrite(&sccb_sw_bus, slv_addr, sizeof(addr), addr) ||
        !i2cdevRead(&sccb_sw_bus, slv_addr, 1, &data)) {
        ESP_LOGE(TAG, "SCCB_Read16 failed addr=0x%02x reg=0x%04x", slv_addr, reg);
        return 0xFF;
    }
    return data;
}
...
{
    uint8_t tx[] = {(uint8_t)(reg >> 8), (uint8_t)reg, data};
    return sccb_sw_result(sccb_sw_initialized &&
                          i2cdevWrite(&sccb_sw_bus, slv_addr, sizeof(tx), tx));
}
...
uint16_t SCCB_Read_Addr16_Val16(uint8_t slv_addr, uint16_t reg)
{
    ...
    if (!sccb_sw_initialized ||
        !i2cdevWrite(&sccb_sw_bus, slv_addr, sizeof(addr), addr) ||
        !i2cdevRead(&sccb_sw_bus, slv_addr, sizeof(data), data)) {
        ESP_LOGE(TAG, "SCCB_Read_Addr16_Val16 failed addr=0x%02x reg=0x%04x", slv_addr, reg);
        return 0xFFFF;
    }
