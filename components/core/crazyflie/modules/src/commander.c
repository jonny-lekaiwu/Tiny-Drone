...
static bool wdtStabilizeLogged;
static bool wdtShutdownLogged;
...
void commanderSetSetpoint(setpoint_t *setpoint, int priority)
{
  ...
  if (priority >= currentPriority) {
    ...
    wdtStabilizeLogged = false;
    wdtShutdownLogged = false;
...
void commanderGetSetpoint(setpoint_t *setpoint, const state_t *state)
{
  ...
  uint32_t currentTime = xTaskGetTickCount();
  const uint32_t setpointAge = currentTime - setpoint->timestamp;

  if (setpointAge > COMMANDER_WDT_TIMEOUT_SHUTDOWN) {
    if (!wdtShutdownLogged) {
      wdtShutdownLogged = true;
      DEBUG_PRINTW("WDT shutdown timeout, age=%lu ms\n",
                   (unsigned long)T2M(setpointAge));
    }
    /* Only a full shutdown disables the active priority. A packet that
     * returns after the 500 ms leveling interval must not latch thrust to
     * zero; after a real shutdown, the existing zero-throttle re-arm rule
     * remains in force. */
  ...
  } else if (setpointAge > COMMANDER_WDT_TIMEOUT_STABILIZE) {
    if (!wdtStabilizeLogged) {
      wdtStabilizeLogged = true;
      DEBUG_PRINTW("WDT stabilize timeout, age=%lu ms\n",
                   (unsigned long)T2M(setpointAge));
    }
    // Leveling ...
  ...
  crtpCommanderRpytApplyYawMode(setpoint, state);
