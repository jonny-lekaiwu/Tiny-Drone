...
static state_t stateSnapshot;
static float thrustSnapshot;
...
extern uint8_t get_flight_mode(void);
...
void stabilizerInit(StateEstimatorType estimator)
{
  ...
  if (camera_init_ok())
  {
    pwm_timmer_set_clock(1000);
  }
  #endif
 
  // Sensors select the flight mode before the motor PWM peripheral is ready.
  // Play the mode indication only after power distribution initialized it.
  if (get_flight_mode() == ALTHOLD_MODE) {
    play_althold_mode();
  } else if (get_flight_mode() == POSHOLD_MODE) {
    play_poshold_mode();
  } 
...
static void stabilizerTask(void* param)
{
  ...
  while(1) {
    ...
    if (testState != testDone) {
      ...
      thrustSnapshot = control.thrust;
      gyroSnapshot = sensorData.gyro;
      ...
      if (emergencyStop &&
        ...
        play_tumble_error();
          ...
          crtpCommanderConsumeLowBatteryAlarmRequest()) {
        play_low_battery_error();
      }
...
bool stabilizerGetStateSnapshot(state_t *snapshot)
{
  if (snapshot == NULL || !isInit) {
    return false;
  }
...
bool stabilizerGetFlightStatusSnapshot(float *thrust, Axis3f *gyro)
{
  if (thrust == NULL || gyro == NULL || !isInit) {
    return false;
  }
  *thrust = thrustSnapshot;
  *gyro = gyroSnapshot;
  return true;
}
