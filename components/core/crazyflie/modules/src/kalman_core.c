...
/* Test guard for zero angular-rate quaternion updates.
 * Set to 0 to restore the previous 0/0 behavior for comparison. */
#define KALMAN_ZERO_ANGLE_GUARD 1
#include "cfassert.h"
...
void kalmanCoreUpdateWithFlow(kalmanCoreData_t* this, const flowMeasurement_t *flow, const Axis3f *gyro)
{
  ...
  if ((now - tick_print) >= pdMS_TO_TICKS(1000))
  {
    tick_print = now;

    snprintf(float_str, sizeof(float_str), "[ns]pre:%.1f,now:%.1f", predictedNX, measuredNX);
    DEBUG_PRINTI("%s", float_str);
  } 
   #endif
...
void kalmanCoreUpdateWithTof(kalmanCoreData_t* this, tofMeasurement_t *tof)
{
  ...
  if ((now - tick_print) >= pdMS_TO_TICKS(1000))
  {
    ...
    snprintf(float_str, sizeof(float_str), "[h] pre:%.1f, now:%.1f", predictedDistance, measuredDistance);
    DEBUG_PRINTI("%s", float_str);
  } 
   #endif
    //Measurement equation
...
void kalmanCorePredict(kalmanCoreData_t* this, float cmdThrust, Axis3f *acc, Axis3f *gyro, float dt, bool quadIsFlying)
{
  ...
  float dq[4];
#if KALMAN_ZERO_ANGLE_GUARD
  if (angle < 1e-6f) {
    /* No measurable rotation: use the identity delta quaternion. */
    dq[0] = 1.0f;
    dq[1] = 0.0f;
    dq[2] = 0.0f;
    dq[3] = 0.0f;
