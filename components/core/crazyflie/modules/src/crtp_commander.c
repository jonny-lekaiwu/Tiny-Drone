...
static bool isInit;
static TickType_t lastPhysicalSetpointTick;

#define WEB_CONTROL_PHYSICAL_GUARD_MS 500
...
static void commanderCrtpCB(CRTPPacket* pk)
{
  ...
  {
      lastPhysicalSetpointTick = xTaskGetTickCount();
                              ...
                              bool carefree)
{
  ...
  if ((xTaskGetTickCount() - lastPhysicalSetpointTick) <
      pdMS_TO_TICKS(WEB_CONTROL_PHYSICAL_GUARD_MS)) {
    return false;
  }
  ...
  CRTPPacket packet = {0};
  packet.port = CRTP_PORT_SETPOINT;
  packet.channel = 0;
  packet.reserved = carefree ? 1U : 0U;
  packet.size = sizeof(values);
  memcpy(packet.data, &values, sizeof(values));

  static setpoint_t webSetpoint;
  crtpCommanderRpytDecodeSetpoint(&webSetpoint, &packet);
  commanderSetSetpoint(&webSetpoint, COMMANDER_PRIORITY_CRTP);
  return true;
}
