...
static volatile bool batteryCriticalForFlight;
static TickType_t batteryFlightCriticalStartTick;

#define PM_FLIGHT_CRITICAL_VOLTAGE 3.0f
#define PM_FLIGHT_CRITICAL_CONFIRM_MS 2000U
...
bool pmIsBatteryCriticalForFlight(void)
{
  return batteryCriticalForFlight;
}
...
void pmTask(void *param)
{
  ...
  batteryFlightCriticalStartTick = 0;
  ...
  if (!batteryCriticalForFlight) {
    if (extBatteryVoltage > 1.0f &&
        extBatteryVoltage < PM_FLIGHT_CRITICAL_VOLTAGE) {
      if (batteryFlightCriticalStartTick == 0) {
        batteryFlightCriticalStartTick = xTaskGetTickCount();
      } else if (T2M(xTaskGetTickCount() -
                     batteryFlightCriticalStartTick) >=
                 PM_FLIGHT_CRITICAL_CONFIRM_MS) {
        /* Latch until reboot. Motor unloading must not cancel landing when
         * the measured battery voltage rises again. */
        batteryCriticalForFlight = true;
        DEBUG_PRINTW("Critical battery confirmed for %u ms: %u mV\n",
                     PM_FLIGHT_CRITICAL_CONFIRM_MS,
                     (unsigned int)extBatteryVoltageMV);
      }
    } else {
      batteryFlightCriticalStartTick = 0;
    }
  ...
  if ((int32_t)(tickCount - nextBatteryReport) >= 0)
  {
    CRTPPacket packet = {0};
    ...
    const bool altitudeHolding =
        crtpCommanderRpytIsAltitudeHoldActive();

    packet.port = CRTP_PORT_SETPOINT_HL;
    packet.channel = 0x01; /* Legacy ESP-Drone battery information channel. */
    packet.reserved = 0;
    packet.data[0] = 0x00; /* Battery voltage field. */
    memcpy(&packet.data[1], &value, sizeof(value));
    /* Keep the legacy five-byte prefix unchanged. New receivers may parse
     * the optional altitude extension; old receivers safely ignore it. */
    ...
    #if 0
    DEBUG_PRINTW("Flight telemetry: queued=%d mode=%u height=%ldmm hold=%u size=%u\n",
                 telemetryQueued,
                 (unsigned int)flightMode,
                 (long)(relativeHeightM * 1000.0f),
                 altitudeHolding ? 1U : 0U,
                 (unsigned int)packet.size);
    #endif
    /* crtpSendPacket() is non-blocking. Retry soon if its queue was busy
     * instead of silently waiting another two seconds. */
    nextBatteryReport = tickCount +
        M2T(telemetryQueued == pdTRUE ? 2000 : 200);
  }
#ifdef DEBUG_EP2
