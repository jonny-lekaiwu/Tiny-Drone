...
{
    CRTPPacket p = {0};
    uint32_t tick_print = xTaskGetTickCount();
    while (1) {
...
#endif 
        {
            ...
            #if 0
            if ((lastPacketTick - tick_print) >= pdMS_TO_TICKS(1000))
            {
                tick_print = lastPacketTick;

                if (p.size > CRTP_MAX_DATA_SIZE)
                {
                    DEBUG_PRINTW("p.size:%d\n", p.size);  
                }

                struct CommanderCrtpLegacyValues *values = (struct CommanderCrtpLegacyValues *)&p.data;
    
                char str_value[64]={0};
                snprintf(str_value, sizeof(str_value), "r:%f,p:%f,y:%f,th:%d\r\n", values->roll, values->pitch, values->yaw, values->thrust); 
                DEBUG_PRINTW("%s\n", str_value);  
            } 
            #endif
        }
