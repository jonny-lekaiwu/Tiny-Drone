...
#define SENSORS_ENABLE_PRESSURE_BAROMETER
 
#ifdef CONFIG_TARGET_TINY_DRONE_MINI_V1_0
#define SENSORS_MPU6050_FLIGHT_DLPF MPU6050_DLPF_BW_42
#else
#define SENSORS_MPU6050_FLIGHT_DLPF MPU6050_DLPF_BW_42
#endif
...
#include "hmc5883l.h"
#include "ms5611.h"
#ifdef SENSORS_ENABLE_PRESSURE_BAROMETER
#include "barometer.h"
#endif
// #include "ak8963.h"
#include "zranger.h"
...
#define CONFIG_USING_DATA_READY_CHECK 0
...
static bool isMs5611Present = false;
#endif
#ifdef SENSORS_ENABLE_PRESSURE_BAROMETER
static barometer_t barometer;
#define BAROMETER_STARTUP_VALID_SAMPLES       8U
#define BAROMETER_STARTUP_TIMEOUT_MS          1500U
#define BAROMETER_STARTUP_MAX_ASL_VARIANCE    0.04f
#endif
...
#ifdef CONFIG_TARGET_TINY_DRONE_MINI_V1_0  
#undef SENSORS_ENABLE_RANGE_VL53L1X 
#undef SENSORS_ENABLE_FLOW_PMW3901
#endif
...
static void sensorsTask(void *param)
{
    ...
    while (1) {
        #if !CONFIG_USING_DATA_READY_CHECK
        vTaskDelay(M2T(1));
        #else
        ...
        #endif
        {
            ...
            uint8_t dataLen = (uint8_t)(SENSORS_MPU6050_BUFF_LEN +
                                        ...
                                        (
#ifdef SENSORS_ENABLE_PRESSURE_MS5611
                                         isMs5611Present ? SENSORS_BARO_BUFF_LEN :
#endif
                                         0));
...
#ifdef SENSORS_ENABLE_PRESSURE_BAROMETER
    if (isBarometerPresent) {
        static TickType_t nextBarometerRead = 0;
        const TickType_t now = xTaskGetTickCount();
        static uint32_t tick_print = 0;

        char print_str[64];

        if ((int32_t)(now - nextBarometerRead) >= 0) 
        {
            float pressurePa = 0.0f;
            float temperatureC = 0.0f;
            nextBarometerRead = now + M2T(80);

            if (barometerRead(&barometer, &pressurePa, &temperatureC)) 
            {
                const float asl = 44330.0f * (1.0f - powf(pressurePa / 101325.0f, 0.19029495f));
                if (isfinite(asl)) {
                    sensorData.baro.pressure = pressurePa * 0.01f;
                    sensorData.baro.temperature = temperatureC;
                    sensorData.baro.asl = asl;

                    #if 0
                    if ((now - tick_print) >= 1000)
                    {
                        tick_print = now; 

                        snprintf(print_str, sizeof(print_str), "p:%.2f, t:%.2f, asl:%.2f", sensorData.baro.pressure, sensorData.baro.temperature, asl);

                        DEBUG_PRINTW("%s", print_str);
                    }
                    #endif
                }
            } 
...
static void sensorsDeviceInit(void)
{
    ...
    positionEstimatorAltitudeEnableRobustFusion(false);
...
#else//defined(CONFIG_TARGET_TINY_DRONE_V1_0)
    // To low DLPF bandwidth might cause instability and decrease agility
    // but it works well for handling vibrations and unbalanced propellers
    // Set output rate (1): 1000 / (1 + 0) = 1000Hz
    mpu6050SetRate(0);
    mpu6050SetDLPFMode(SENSORS_MPU6050_FLIGHT_DLPF);
...
#ifdef SENSORS_ENABLE_PRESSURE_BAROMETER
    uint8_t barometerId = 0;
    /* The pressure sensor shares the sensor I2C bus and pins with MPU6050. */
    if (barometerInit(&barometer, I2C0_DEV, BAROMETER_I2C_ADDR_SDO_LOW, &barometerId)) {
        ...
        float lastAsl = 0.0f;
        const TickType_t validationStart = xTaskGetTickCount();

        while (validSamples < BAROMETER_STARTUP_VALID_SAMPLES &&
               (xTaskGetTickCount() - validationStart) <
                   M2T(BAROMETER_STARTUP_TIMEOUT_MS)) {
            float pressurePa = 0.0f;
            float temperatureC = 0.0f;

            if (barometerRead(&barometer, &pressurePa, &temperatureC)) {
                const float asl = 44330.0f *
                    (1.0f - powf(pressurePa / 101325.0f, 0.19029495f));
                if (isfinite(asl)) {
                    validSamples++;
                    const float delta = asl - aslMean;
                    aslMean += delta / (float)validSamples;
                    aslM2 += delta * (asl - aslMean);
        ...
        const float aslVariance = validSamples > 1U ?
            aslM2 / (float)(validSamples - 1U) : INFINITY;
        if (validSamples == BAROMETER_STARTUP_VALID_SAMPLES &&
            isfinite(aslVariance) &&
            aslVariance <= BAROMETER_STARTUP_MAX_ASL_VARIANCE) {
            sensorData.baro.pressure = lastPressurePa * 0.01f;
            sensorData.baro.temperature = lastTemperatureC;
            sensorData.baro.asl = lastAsl;
            isBarometerPresent = true;
            positionEstimatorAltitudeEnableRobustFusion(true);
            DEBUG_PRINTI("Barometer validation [OK], ID=0x%02X, samples=%lu, variance=%.5f m2, P=%.2f hPa, T=%.2f C, ASL=%.2f m.\n",
                         barometerId, (unsigned long)validSamples,
                         (double)aslVariance,
                         (double)sensorData.baro.pressure,
                         (double)lastTemperatureC, (double)lastAsl);
        } else {
            DEBUG_PRINTW("Barometer validation [FAIL], ID=0x%02X, samples=%lu/%u, variance=%.5f m2; altitude hold disabled.\n",
                         barometerId, (unsigned long)validSamples,
                         BAROMETER_STARTUP_VALID_SAMPLES,
                         (double)aslVariance);
        }
    } else {
        DEBUG_PRINTW("Barometer I2C connection [FAIL], address=0x%02X, ID=0x%02X.\n",
                     BAROMETER_I2C_ADDR_SDO_LOW, barometerId);
    }
#endif
    ...
    if (isVl53l1xPresent) {
        setCommandermode(isPmw3901Present ? POSHOLD_MODE : ALTHOLD_MODE);
    }
#ifdef SENSORS_ENABLE_PRESSURE_BAROMETER
    else if (isBarometerPresent) {
        /* The existing altitude estimator fuses barometer height with the
         * gravity-compensated accelerometer vertical velocity. The existing
         * position PID then supplies altitude and vertical-speed cascades. */
...
static void sensorsInterruptInit(void)
{
    ...
    gpio_reset_pin(GPIO_INTA_MPU6050_IO);
...
static bool sensorsFindBiasValue(BiasObj *bias)
{
    ...
    if (bias->isBufferFilled) {
        ...
        if (varianceSampleTime + GYRO_MIN_BIAS_TIMEOUT_MS < xTaskGetTickCount())
        {
            ...
            char float_str[32]={0};
            snprintf(float_str, sizeof(float_str),"%f", bias->variance.x);
            DEBUG_PRINTW("bias->variance.x=%s\n", float_str);
            snprintf(float_str, sizeof(float_str),"%f", bias->variance.y);
            DEBUG_PRINTW("bias->variance.y=%s\n", float_str);
            snprintf(float_str, sizeof(float_str),"%f", bias->variance.z);
            DEBUG_PRINTW("bias->variance.z=%s\n", float_str);
        }
        #else
        if (bias->variance.x < GYRO_VARIANCE_THRESHOLD_X &&
                bias->variance.y < GYRO_VARIANCE_THRESHOLD_Y &&
